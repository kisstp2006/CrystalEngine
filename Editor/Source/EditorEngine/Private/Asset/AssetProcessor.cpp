...
namespace CE
{
    ...
    void AssetProcessor::ImportAssets(const Array<IO::Path>& sourceAssets)
    {
    	...
    	if (IsInProgress())
    	{
    		importQueue.AddRange(sourceAssets);
    		return;
    	}
    	...
    	for (const IO::Path& path : sourceAssets)
    	{
    		if (path.IsDirectory())
                return;
    ...
    void AssetProcessor::Tick()
    {
    	...
    	if (!IsInProgress() && importQueue.NotEmpty())
    	{
    		ImportAssets(importQueue);
